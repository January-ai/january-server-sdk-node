...
try {
  ...
  assert.deepEqual(await client.clientTokens.create({ endUserId: " user ", scopes: ["foods:read"], ttlSeconds: 600 }), { token: "ct-fixture", expiresIn: 300 });
  ...
  await client.clientTokens.create({ endUserId: "user" });
  assert.deepEqual(requests[1].body, { end_user_id: "user" });
  ...
  await assert.rejects(client.clientTokens.create({ endUserId: "user" }), error => error.status === 429 && error.code === "rate_limited");
